...
function BusinessItem({ business, showDirection=false }) {
    ...
    const onDirectionClick=()=>{
      ...
      +business.geometry.location.lat
      +','+business.geometry.location.lng+'&travelmode=transit')
  }
  ...
  />
      <h2 className='text-[13px] font-bold mt-1 line-clamp-1'>{business.name}</h2>
            ...
            {showDirection? 
              ...
              <h2 className='text-[#0075ff] flex justify-between items-center'> {distance} mile <span className='border-[1px] p-1 rounded-full border-blue-500 hover:text-white hover:bg-blue-500' onClick={()=>onDirectionClick()}>Get directions</span></h2>
            </div> : null } 
    </div>
  )
}
